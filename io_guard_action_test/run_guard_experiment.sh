...
wait_for_module_idle()
{
	...
	while [ "$tries" -lt 5 ]; do
		...
		case "$stats" in
			*'current_depth:0 reserved_depth:0 issued_depth:0 '*'inuse=0'*) return ;;
		esac
		tries=$((tries + 1))
		sleep 1
	done
...
require_counters_stable()
{
	...
	if [ "$counter_now_inference" != "$counter_base_inference" ] ||
	   ...
	   [ "$counter_now_allocations" != "$counter_base_allocations" ]; then
		echo "module counter changed outside treatment" >&2
...
while IFS=, read -r block position profile read_guard write_guard seed; do
	...
	require_counters_stable
	cat "$proc_root/stats" > "$prefix-stats-before.txt"
	...
	set +e
	timeout 20 "$bin" run --profile "$profile" --read-path "$read_file" \
		--write-path "$write_file" --csv "$prefix.csv" --file-mb 128 \
		--warmup-ms 500 --measure-ms 2000 \
	...
	grep -q ' ret=0$' "$prefix-stdout.txt" || {
		echo "trial did not report ret=0: $trial" >&2
		exit 1
	}
	wait_for_module_idle
	require_counters_stable
	cat "$proc_root/stats" > "$prefix-stats-after.txt"
...
wait_for_module_idle
require_counters_stable
cat "$proc_root/stats" > "$output_dir/stats-after.txt"

...
if [ "$#" -ne 3 ]; then
	echo "usage: $0 SCHEDULE_CSV OUTPUT_DIR CELL_QUOTA" >&2
	exit 2
fi
...
last_block=0
last_idle_stats=
read_state_threshold_ns=2207192
write_state_threshold_ns=9090039

case "$cell_quota" in
	''|*[!0-9]*) echo "invalid cell quota: $cell_quota" >&2; exit 2 ;;
esac
[ "$cell_quota" -le 32 ] || {
	echo "cell quota is too large: $cell_quota" >&2
	exit 2
}
...
quota_complete()
{
	awk -F, -v quota="$cell_quota" '
		...
		NR > 1 { counts[$3 SUBSEP $4]++ }
		END {
			for (p = 1; p <= 4; p++)
				for (s = 1; s <= 4; s++)
					if (counts[profiles[p] SUBSEP states[s]] < quota)
						exit 1
		}
	' "$output_dir/enrollment.csv"
}
...

{
	...
	echo "read_state_threshold_ns=$read_state_threshold_ns"
	echo "write_state_threshold_ns=$write_state_threshold_ns"
	echo "cell_quota=$cell_quota"
...
echo "block,position,profile,state,pre_fg_p99_ns,pre_bg_read_p99_ns,pre_bg_write_p99_ns" \
	> "$output_dir/enrollment.csv"

tail -n +2 "$schedule" |
while IFS=, read -r block position profile read_guard write_guard \
	read_limit write_limit seed; do
	[ -n "$block" ] || continue
	case "$profile" in
		BASELINE|READ_D2|WRITE_D2|ALL_D2) ;;
		*) echo "invalid profile in schedule: $profile" >&2; exit 1 ;;
	esac
	case "$profile:$read_guard:$write_guard:$read_limit:$write_limit" in
		BASELINE:0:0:4:4|READ_D2:1:0:2:4|WRITE_D2:0:1:4:2|ALL_D2:1:1:2:2) ;;
		*) echo "limits do not match profile: $profile $read_limit $write_limit" >&2; exit 1 ;;
	esac
	...
	timeout 20 "$bin" run-state --profile "$profile" \
		...
		--read-state-threshold-ns "$read_state_threshold_ns" \
		--write-state-threshold-ns "$write_state_threshold_ns" \
	...
	state_line=$(grep '^pre_state=' "$prefix-stdout.txt") || {
		echo "trial did not report pre-state: $trial" >&2
		exit 1
	}
	pre_state=$(echo "$state_line" | sed -n 's/^pre_state=\([^ ]*\).*/\1/p')
	pre_fg_p99_ns=$(echo "$state_line" | sed -n 's/.* pre_fg_p99_ns=\([0-9][0-9]*\).*/\1/p')
	pre_bg_read_p99_ns=$(echo "$state_line" | sed -n 's/.* pre_bg_read_p99_ns=\([0-9][0-9]*\).*/\1/p')
	pre_bg_write_p99_ns=$(echo "$state_line" | sed -n 's/.* pre_bg_write_p99_ns=\([0-9][0-9]*\).*/\1/p')
	case "$pre_state" in
		NORMAL|READ_SLOW|WRITE_SLOW|MIXED_SLOW) ;;
		*) echo "invalid pre-state for $trial: $pre_state" >&2; exit 1 ;;
	esac
	for state_value in "$pre_fg_p99_ns" "$pre_bg_read_p99_ns" \
		"$pre_bg_write_p99_ns"; do
		[ -n "$state_value" ] || {
			echo "missing pre-state P99 for $trial" >&2
			exit 1
		}
	done
	...
	echo "$block,$position,$profile,$pre_state,$pre_fg_p99_ns,$pre_bg_read_p99_ns,$pre_bg_write_p99_ns" \
		>> "$output_dir/enrollment.csv"
	echo "completed $trial"
	sleep 2
	if [ "$cell_quota" -gt 0 ] && [ $((block % 4)) -eq 0 ] &&
	   [ "$position" -eq 4 ] && quota_complete; then
		echo "quota complete after block $block"
		echo "$block" > "$output_dir/quota-complete-block.txt"
		break
	fi
done
...
if [ "$cell_quota" -gt 0 ] &&
   [ ! -s "$output_dir/quota-complete-block.txt" ]; then
	echo "quota-incomplete" > "$output_dir/status.txt"
	echo "cell quota was not reached before schedule exhaustion" >&2
	exit 1
fi
